...
}
{
    ...
    let graph1 = B.functiongraph((x) => 4 * Math.sin(x) + 4, 0, 4 * Math.PI, { name: 'sin(x)', withLabel: true, strokeColor: 'red' });
    let graph2 = B.functiongraph((x) => Math.pow(x - 4, 2) - 2, 0, 10, { name: 'x^2 - 2', withLabel: true, strokeColor: 'green' });
}
{
...
}
{
    ...
    let c1 = B.curve(function (t) { return t; }, function (t) { return t * t * t; });
...
}
{
    ...
    ///////////////
    let f = B.functiongraph((x) => Math.cos(x) + 10);
...
}
{
    ...
    let c1 = JSX.create('circle', [[0, -3], [2, -3]]), p1 = JSX.point([-4, -2]), g1 = JSX.glider(2, -2, c1), s1 = JSX.segment(g1, p1), p2 = JSX.midpoint(s1), curve = JSX.tracecurve(g1, p2);
}
{
...
}
{
    ...
    let f = (x, y) => x ** 2 - 2 * x * y - 2 * x + (a.Value() + 1) * y ** 2 + (4 * a.Value() + 2) * y + 4 * a.Value() - 3;
...
}
{
    ...
    B.print('Vectorfield');
    // Defining functions
    let fx = (x, y) => Math.sin(y);
    let fy = (x, y) => Math.cos(x);
    let field = B.vectorfield([fx, fy], // Defining function
...
}
{
    ...
    let f = board.functiongraph((x) => 3 * Math.sin(x)), g = board.glider(1, 2, f), t = board.tangent(g);
    let st = board.slopetriangle(t);
    board.text(0, 4, () => 'Slope: ' + st.Value().toFixed(2));
    // Create a on a line and a point on that line
    let p1 = board.point([-3, 3]), p2 = board.point([1, -3]), li = board.line(p1, p2), p = board.glider(0, 0, li);
...
}
{
    ...
    let c1 = view.curve3D((x) => Math.sin(x), (y) => 3 * Math.cos(y), (z) => z, [-5, 5]);
}
{
    ...
    let F = (x, y) => Math.sin(x * y / 4);
    // 3D surface
    let c = view.functiongraph3D(F, box, // () => [-s.Value()*5, s.Value() * 5],
...
}
{
    ...
    // Sphere
    let c = view.parametricsurface3D((u, v) => 2 * Math.sin(u) * Math.cos(v), (u, v) => 2 * Math.sin(u) * Math.sin(v), (u, v) => 2 * Math.cos(u), [0, 2 * Math.PI], [0, Math.PI], {
        strokeColor: '#ff0000',
        stepsU: 30,
        stepsV: 30
    });
...
}
{
    ...
    let s = brd2.slider([1, 7], [4, 7], [0, 0, 50], {
        name: 'P'
    });
    let x = 4;
    let y = 3;
...
}
{
    ...
    let cb = board.curve((t) => Math.cos(t) + c1.Value() * Math.cos(f1.Value() * t) + c2.Value() * Math.cos(f2.Value() * t), (t) => Math.sin(t) + c1.Value() * Math.sin(f1.Value() * t) + c2.Value() * Math.sin(f2.Value() * t), 0, 2.02 * Math.PI, { strokeWidth: 4, strokeColor: 'blue', shadow: true });
    let cw = board.create('curve', [
        (t) => Math.cos(t) + c1.Value() * Math.cos(f1.Value() * t) + c2.Value() * Math.cos(f2.Value() * t),
        (t) => Math.sin(t) + c1.Value() * Math.sin(f1.Value() * t) + c2.Value() * Math.sin(f2.Value() * t),
        0, 2.02 * Math.PI
...
}
{
    ...
    let p1 = b.point([-1, 4]), p2 = b.point([4, 1]), q1 = b.point([-2, -3]), q2 = b.point([4, 3]), li1 = b.line(p1, p2, { strokeColor: 'black', lastArrow: true }), li2 = b.line(q1, q2, { lastArrow: true }), a1 = b.angle(li1, li2, [5.5, 0], [4, 3], { radius: 1 }), a2 = b.angle(li1, li2, 1, -1, { radius: 2 });
}
{
...
}
{
    ...
    let p1 = board.point([4.0, 0]), p2 = board.point([0.0, -0.5]), p3 = board.point([-0.5, 4.0]);
...
}
{
    ...
    let getMouseCoords = function (e, i = 0) {
        var cPos = JSX.board.getCoordsTopLeftCorner(e, i), absPos = window.JXG.getPosition(e, i), dx = absPos[0] - cPos[0], dy = absPos[1] - cPos[1];
        let coords = new window.JXG.Coords(1, [dx, dy], JSX.board);
...
}
{
    ...
    let p1 = JSX.point([-2, -2], { snapToGrid: true }), p2 = JSX.point([3, 3], { snapToGrid: true }), p3 = JSX.point([function () { return p1.X(); }, function () { return p2.Y(); }], { color: 'blue', snapToGrid: true }), p4 = JSX.point([function () { return p2.X(); }, function () { return p1.Y(); }], { color: 'blue', snapToGrid: true });
